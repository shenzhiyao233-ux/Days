...
class DaysWidget : GlanceAppWidget() {
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        val appId=(id as AppWidgetId).appWidgetId
...
@Composable private fun WidgetContent(context:Context,event:DayEvent?,binding:WidgetBinding?){
    ...
    val base=when(binding.backgroundType){BackgroundType.SYSTEM->ColorProvider(Color(0xFFF5F5F5),Color(0xFF242424));BackgroundType.SOLID->ColorProvider(Color(binding.backgroundColor));BackgroundType.TRANSLUCENT->ColorProvider(Color(binding.backgroundColor).copy(alpha=binding.backgroundAlpha));BackgroundType.PHOTO->ColorProvider(Color(0xFF30343B))}
